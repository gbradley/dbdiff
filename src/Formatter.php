...
class Formatter {
	 ...
	 */
	public function format($id, array $diff_src = null, array $diff_dest = null, array $row_src = null, array $row_dest = null, string $table_src, string $table_dest) : string {
		...
		$columns = array_keys(is_array($diff_src) ? $diff_src : $diff_dest);
		foreach ($columns as $column) {
			
			// Get the value from each row.
			$value_a = $diff_src ? $this->formatValue($diff_src[$column]) : null;
			$value_b = $diff_dest ? $this->formatValue($diff_dest[$column]) : null;
			...
			$rows[2] .= ' | ' . $this->colorize($diff_src ? str_pad($value_a, $column_len) : str_repeat('-', $column_len), $this->color_src);
			$rows[3] .= ' | ' . $this->colorize($diff_dest ? str_pad($value_b, $column_len) : str_repeat('-', $column_len), $this->color_dest);
		}
